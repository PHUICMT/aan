...
<div class="reader-bar" data-test="reader-toolbar">
  ...
  <div class="sep"></div>
  <button class="mode" onclick={onCycleMode} use:tooltip={"{t('reader.mode.title')} — {t('reader.mode.desc')}"}>
    <Icon name={modeIcon} size={12} />
    {modeLabel}
  </button>
  <button class="mode" onclick={onCycleFit} use:tooltip={"{t('reader.fit.title')} — {t('reader.fit.desc')}"}>
    <Icon name={fitIcon} size={12} />
    {fitLabel}
  </button>
  <div class="zoom-ctrl" use:tooltip={"{t('reader.zoom.title')} — {t('reader.zoom.desc')}"}>
    <button class="mode zoom-btn" onclick={onZoomOut} aria-label="Zoom out">−</button>
    <button class="mode zoom-label" onclick={onZoomReset} use:tooltip={"Reset zoom"}>{Math.round(zoom * 100)}%</button>
    <button class="mode zoom-btn" onclick={onZoomIn} aria-label="Zoom in">+</button>
  </div>
